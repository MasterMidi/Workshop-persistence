...
public class DBProduct implements IProductDB {
	...
	private SellableProduct buildObject(ResultSet rs) throws SQLException, DataAccessException {
		...
		case 1:
			...
			currentProduct = new Clothing(rs.getString("name"), rs.getString("description"), rs.getInt("ean"),
					rs.getDouble("purchase_price"), rs.getString("country_of_origin"),
					rs.getDate("bought_date").toLocalDate(), rs.getInt("clothing_size"), rs.getString("clothing_color"),
					emptySupplier, getProductPrices(rs.getInt("id")));
			break;
		case 2:
			currentProduct = new GunReplica(rs.getString("name"), rs.getString("description"), rs.getInt("ean"),
					rs.getDouble("purchase_price"), rs.getString("country_of_origin"),
					rs.getDate("bought_date").toLocalDate(), rs.getInt("gun_replica_caliber"),
					rs.getString("gun_replica_material"), emptySupplier, getProductPrices(rs.getInt("id")));
			break;
		case 3:
			currentProduct = new Equipment(rs.getString("name"), rs.getString("description"), rs.getInt("ean"),
					rs.getDouble("purchase_price"), rs.getString("country_of_origin"),
					rs.getDate("bought_date").toLocalDate(), rs.getString("equipment_type"), emptySupplier,
					getProductPrices(rs.getInt("id")));
			break;
		default:
		}
